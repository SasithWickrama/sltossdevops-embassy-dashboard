...
export class TotalContainerComponent implements OnInit {
  ...
  allTotalSubscription: Subscription;
  ...
  constructor(private dataService:DataService, private storageService:StorageService, private router:Router) {
    ...
    /**   call at contructor - get total all appointment count**/
    this.triggerAllTotalSubcription();
  ...
  /** Subscription get all appointment appointment count   from dataService layer**/
  triggerAllTotalSubcription(){
    this.allTotalSubscription = this.dataService.getAllAppointmentsCount().subscribe(x => {
      this.allTotal = x
    });
  }
